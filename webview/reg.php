<?php
	...
	if(isset($_POST['name']) and isset($_POST['lname']) and isset($_POST['email']) and isset($_POST['username']) and isset($_POST['password_1']) and isset($_POST['password_2'])){
		...
		if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) === false){
			$selectEmail = $db->prepare("SELECT email FROM client WHERE email = '".$_POST['email']."'");
			$selectEmail->execute();
			if($emailFound = $selectEmail->fetch()){
				...
				$selectUsername = $db->prepare("SELECT username FROM client WHERE username = '".$_POST['username']."'");
